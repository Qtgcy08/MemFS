...
async function test() {
    ...
    // Full search
    console.log('Full search result:');
    const result = searcher.search(query);
    console.log('  results.length:', result.results.length);
    console.log('  entities:', result.results.map(e => e.entityName).join(', '));
    console.log('  totalCandidates:', result.stats.totalCandidates);

    // Check each term manually
    console.log('\nChecking each term:');
    const terms = ["xyz123nonexistent", "xyz1", "yz12", "z123", "123n", "23no", "3non", "none", "onex", "nexo", "exis", "xist", "iste", "sten", "tenx", "enxt", "nxts"];
    terms.forEach(term => {
        const tr = searcher.searchTerm(term);
        console.log(`  "${term}": tfidf=${tr.tfidfCount}, fuse=${tr.fuseCount}`);
    });

    // Also check "微软"
    console.log('\nChecking "微软":');
    const micros = searcher.searchTerm("微软");
    console.log(`  "微软": tfidf=${micros.tfidfCount}, fuse=${micros.fuseCount}`);
    micros.tfidfResults.forEach(r => {
        console.log(`    TF-IDF: ${r.entityName}, score=${r.score.toFixed(4)}`);
    });
