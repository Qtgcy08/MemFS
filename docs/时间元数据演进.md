...
### 2.1 阶段一：简单字符串格式（最初实现）

```jsonl
{"type":"observation","id":1,"content":"研究笔记","createdAt":"2024-01-15"}
```

**问题**：

- 无时区信息，无法区分 UTC 和本地时间
- 格式不统一，有的带时区偏移，有的不带
- 无法区分创建时间和修改时间

### 2.2 阶段二：UTC ISO 8601 与本地+偏移混用
...
1. **精确性**：`Asia/Shanghai` 明确指向中国上海，而 `+0800` 可能代表北京、香港、新加坡、吉隆坡等多个地区

2. **语义丰富性**：时区名称包含地理和文化信息。上海和香港虽同属 `+0800`，但学术传统和研究视角可能不同
...
- **存储层**：UTC + IANA 时区双记录，兼顾精确性与语义完整性
...
2. **时间线可视化**：按时间顺序展示知识演化历程
3. **时区感知搜索**：支持按时间范围筛选时区
4. **知识流动分析**：分析跨时区的知识引用模式

### 5.2 潜在挑战

1. **性能考虑**：完整版本历史可能带来存储压力
2. **数据迁移**：现有数据的渐进式升级需要清晰的迁移策略
3. **用户体验**：时间元数据的复杂性需要良好的 UI 抽象
...
> **参考文档**：
> 
> - [AGENTS.md - 时间戳处理规范](../../AGENTS.md#timestamp-handling)
