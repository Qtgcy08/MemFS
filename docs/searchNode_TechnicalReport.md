...
| 字段          | 权重  | 设计理由                                                |
| ----------- | --- | --------------------------------------------------- |
| name        | 5.0 | 实体名称是实体的核心标识，通常高度凝练地表达实体概念。匹配名称应该获得最高的相关性得分。        |
| entityType  | 4.0 | 实体类型表示实体的类别信息，如"人物"、"地点"、"概念"等。类型的匹配对于限定搜索范围具有重要作用。 |
| definition  | 4.0 | 定义是实体的详细描述，包含丰富的语义信息，其重要性与实体类型相当。                   |
| observation | 3.0 | 观察内容是实体的补充信息，数量可能较多但通常包含用户最关心的具体内容，权重与定义相当。         |
...
**数量限制（limit）**：默认限制返回 15 个结果，这是根据用户体验设计的参数。返回过多结果可能导致LLM上下文溢出，而返回过少可能遗漏重要信息。15 个结果通常足以覆盖用户的查询需求，同时保持响应的高效性。
